...
func handleActivityEnd(bot *linebot.Client, event *linebot.Event, State string) {
	...
	// คำนวณระยะเวลา
	// duration := endTimeStr.Sub(startTimeStr)
	// durationStr := fmt.Sprintf("%d ชั่วโมง %d นาที", int(duration.Hours()), int(duration.Minutes())%60)
	...
	startTimeStr := strings.TrimSpace(message.Text)
	flexMessage := flexmessage.FormatActivityEndConfirmation(startTimeStr,endTimeStr)
	if _, err := bot.ReplyMessage(event.ReplyToken, flexMessage).Do(); err != nil {
		log.Println("❌ Error sending Flex Message:", err)
	}
...
func calculateDuration(startTimeStr, endTimeStr string) string {
	// Assuming startTimeStr and endTimeStr are in the same format
	startTime, err := parseTimeInput(startTimeStr)
	if err != nil {
	...
	// Return the duration as a string in a readable format (e.g., "2 hours 30 minutes")
